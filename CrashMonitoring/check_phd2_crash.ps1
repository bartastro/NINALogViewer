...
if ($CrashEvent) {
    ...
    # Controleer of PHD2 de dader was
    if ($EventMessage -like "*phd2*") {
       
        # Tijdstip mooi formatteren
        $Tijdstip = $CrashEvent.TimeCreated.ToString("dd-MM-yyyy HH:mm:ss")
		
		$Titel = "!! PHD2 crash gedetecteerd !!"

		if ($EventMessage -like "*werfault.exe") { $Titel = "PHD2 detector -- Test OK"}
        ...
        $Body = @{
            embeds = @(
                @{
                    title       = $Titel
                    description = "PHD2 is zojuist onverwacht gecrasht."
